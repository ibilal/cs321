...
class Ast {
  ...
  public static class ClassDecl extends Node {
    ...
    void checkVarInit(VarSet initSet) throws Exception {
        VarSet initSet2 = new VarSet();
        for(VarDecl v: flds)
            initSet = VarSet.add(initSet, v.nm);
        for(MethodDecl m: mthds)
  ...
  public static class MethodDecl extends Node {
    ...
    public final Stmt[] stmts;	    // method bod
    ...
    boolean checkReach(boolean reachable) throws Exception {
        ...
        {
            reachable = stmts[i].checkReach(reachable);
            if(!reachable){
                try{    
                    if(i <= (length-2)){
                        reachable = stmts[i+1].checkReach(reachable);
                    }
                    else
                        return reachable;
                } 
    ...
    void checkVarInit(VarSet initSet) throws Exception {
        VarSet paramsV = new VarSet();
        for(Param p: params){
            paramsV = VarSet.add(initSet, p.nm);
            //paramsV = initSet.add(initSet, p.nm);
        }
        VarSet varsV = new VarSet();
        for(VarDecl v: vars){
            if(v.init != null){
                varsV = VarSet.add(paramsV, v.nm);
                //varsV = paramsV.add(paramsV, v.nm);
            }
        }
        varsV = VarSet.union(paramsV, varsV);
        for(Stmt s: stmts)
            varsV = s.checkVarInit(varsV);
    }
  }
  ...
  public static class Assign extends Stmt {
    ...
    VarSet checkVarInit(VarSet initSet)throws Exception {
        ...
        if(lhs instanceof Id){
            Id something = (Ast.Id)lhs;
            initSet = initSet.add(initSet, something.nm);
            return initSet;
        }
  ...
  public static class If extends Stmt {
    ...
    boolean checkReach(boolean reachable) throws Exception {
      boolean ifStatus=false, elseStatus=false;
      if (!reachable) 
	throw new StaticError("Unreachable statement: " + this);
      ifStatus = s1.checkReach(reachable);
      if (s2 != null){
            elseStatus = s2.checkReach(reachable);
            return ifStatus && elseStatus;
      }
      else 
          return ifStatus;
    }
    ...
    VarSet checkVarInit(VarSet initSet)throws Exception {
        ...
        VarSet ifS = s1.checkVarInit(initSet);

        VarSet result = new VarSet();
        
        if(s2 != null){
            VarSet elseS = s2.checkVarInit(initSet);
        
            result = VarSet.intersect(ifS, elseS);
